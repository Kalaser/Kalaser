...
def generate_markdown(repos):
    md = '<p align="center">\n'
    for repo in repos:
        md += f'  <a href="{repo["link"]}" target="_blank">\n'
        md += f'    <span style="display:inline-block; background-color:#f3f4f6; color:#0366d6; border-radius:9999px; padding:6px 12px; margin:4px; font-size:13px;">\n'
        md += f'      {repo["name"]} · {repo["author"]} · {repo["lang"]}\n'
        md += f'    </span>\n'
        md += f'  </a>\n'
    md += '</p>'
    return md
