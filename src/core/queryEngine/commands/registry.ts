...
export async function* handleHooksCommand(
  ...
  if (report.globallyDisabled) {
    lines.push("⚠ KALASER_DISABLE_HOOKS is set — all hooks are disabled this session.");
    lines.push("");
  }
  ...
  lines.push("Set KALASER_DISABLE_HOOKS=1 to disable every hook for one session.");
