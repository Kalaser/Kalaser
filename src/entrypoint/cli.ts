...
async function main(): Promise<void> {
  if (process.argv.includes("--version") || process.argv.includes("-v")) {
    console.log("kalaser v" + VERSION);
    process.exit(0);
  }

  if (process.argv.includes("--help") || process.argv.includes("-h")) {
    console.log(`
kalaser v${VERSION} — Terminal-native agentic coding system
...
Agent Teams (stage 21 — requires --agent-teams or KALASER_TEAMS=1):
...
Hooks (stage 22 — user-defined shell scripts on lifecycle events):
  ...
  Set KALASER_DISABLE_HOOKS=1 to disable all hooks globally.
