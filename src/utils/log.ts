...
export function debugLog(scope: string, message: string, details?: Record<string, unknown>): void {
  if (!process.env.KALASER_DEBUG) return;
  const timestamp = new Date().toISOString();
  const suffix = details ? ` ${JSON.stringify(details)}` : "";
  console.error(`[kalaser][${timestamp}][${scope}] ${message}${suffix}`);
}
 ...
 * KALASER_DEBUG=1, but it must NOT corrupt Ink's stdout-rendered UI.
 *
...
export function logWarn(message: string): void {
  ...
  console.error(`[kalaser][warn] ${message}`);
}
