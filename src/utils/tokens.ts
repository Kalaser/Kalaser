...
export function getContextWindowForModel(model: string): number {
  // KALASER_MAX_CONTEXT_TOKENS is the project name; CLAUDE_CODE_MAX_CONTEXT_TOKENS
  // is honored too for parity with source (matches getMaxRetries' alias pattern).
  const envOverride =
    process.env.KALASER_MAX_CONTEXT_TOKENS ?? process.env.CLAUDE_CODE_MAX_CONTEXT_TOKENS;
  if (envOverride) {
