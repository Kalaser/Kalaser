...
async function main(): Promise<void> {
  ...
  await withTempHome(async (_home, cwd) => {
    ...
    // env var KALASER_PROJECT_DIR is exposed
    const envResult = await executeHookCommand({
      hook: { type: "command", command: "printenv KALASER_PROJECT_DIR" },
      hookEvent: "PreToolUse",
    ...
    assert(
      ...
      "KALASER_PROJECT_DIR env var matches cwd",
    );
  });
  ...
  console.log("\n[9] kill-switch — KALASER_DISABLE_HOOKS");
  ...
  await withTempHome(async (home, cwd) => {
    ...
    const prevKill = process.env["KALASER_DISABLE_HOOKS"];
    try {
      process.env["KALASER_DISABLE_HOOKS"] = "1";
      ...
      if (prevKill !== undefined) process.env["KALASER_DISABLE_HOOKS"] = prevKill;
      else delete process.env["KALASER_DISABLE_HOOKS"];
    }
  });
