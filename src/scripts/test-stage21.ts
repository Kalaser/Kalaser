...
async function withTeamsFlag<T>(
  ...
  const prevEnv = process.env["KALASER_TEAMS"];
  if (enabled) process.env["KALASER_TEAMS"] = "1";
  else delete process.env["KALASER_TEAMS"];
  try {
    return await fn();
  } finally {
    if (prevEnv !== undefined) process.env["KALASER_TEAMS"] = prevEnv;
    else delete process.env["KALASER_TEAMS"];
  }
}
